...
def check(r):
    ...
    b5 = 'byr' in r
    b6 = 'iyr' in r
    b7 = 'cid' in r # optional
    b8 = 'hgt' in r
    return b1 and b2 and b3 and b4 and b5 and b6 and b8

import functools
def check_all_records(rr):
    return map(check,rr)
    
def count_tt(bb):
    tt = filter(lambda x: x == True, bb)
    return len(list(tt))

def day4part1(i):
    return count_tt(check_all_records(mkdict(i)))
...
#    print('check {}'.format(r))
    ...
    pid = r['pid']
#    cid = r['cid']
    byr_b = int(byr) >= 1920 and int(byr) <= 2002
    iyr_b = int(iyr) >= 2010 and int(iyr) <= 2020
    eyr_b = int(eyr) >= 2020 and int(eyr) <= 2030
    if 'cm' in hgt:
        l = int(hgt.replace('cm',''))
        hgt_b = l >= 150 and l <= 193
    elif 'in' in hgt:
        l = int(hgt.replace('in',''))
        hgt_b = l >= 59 and l <= 76
    else:
        hgt_b = False
    hcl_b = False
    ...
    ecl_b = ecl == 'amb' or ecl == 'blu' or ecl == 'brn' or ecl == 'gry' or ecl == 'grn' or ecl == 'hzl' or ecl == 'oth'
    pid_b = False
    x = re.search('[0-9]{9}', pid)
    if x is not None:
        pid_b = x[0] == pid
        
    return byr_b and iyr_b and eyr_b and hgt_b and hcl_b and ecl_b and pid_b
                
def check_then_valid(r):
    return check(r) and valid(r)
    
def checkvalid_all_records(rr):
    return list(map(check_then_valid, rr))

def day4part2(i):
    return count_tt(checkvalid_all_records(mkdict(i)))
